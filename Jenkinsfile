  ...
  stages {
    stage('Build') {
      parallel {
        ...
        stage('Upload Config') {
          steps {
            SWEAGLEUpload(actionName: 'uploadConfig', fileLocation: '/Users/boondock/.jenkins/workspace/LocalWebHost_master/InfraDB.json', format: 'json', nodePath: 'MyNodePath', allowDelete: true, tag: 'Version_${COMMIT}', description: 'DataUpload', withSnapshot: true, markFailed: true, showResults: true, onlyParent: true)
          }
        }
      }
    }
